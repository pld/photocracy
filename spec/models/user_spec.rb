 ...
 if REST_AUTH_SITE_KEY.blank?
  ...
  describe 'responses count' do
    ...
    before do
      ...
      Response.delete_all
      @visits.times do
        visit = Visit.create!(:ip_address => '11', :user => @user)
        @responses.times { Response.create!(:prompt_id => count += 1, :visit => visit, :vote_id_ext => count, :response_time => 15) }
      end
    end

    it 'should count all reponses' do
      @user.responses_count.should == @visits * @responses
    end
  end

  describe 'rank responses' do
    fixtures :users
    ...
    before do
      ...
      @users.each do |user|
        ...
        @visits.times do
          visit = Visit.create(:ip_address => '11', :user => user)
          @responses.times { Response.create(:prompt_id => count += 1, :visit => visit, :vote_id_ext => count += 1, :response_time => 15) }
        end
      end
    end

    it 'should rank first user as last' do
      User.responses_rank(User.first).should == User.count
    end

    it 'should rank last user as first' do
      User.responses_rank(User.last).should == 1
    end
  end

  describe 'register voter' do
    fixtures :users
    ...
    it 'should do nothing if ext id set' do
      lambda { @user.register_voter }.should_not change(@user, :voter_id_ext)
    end

    it 'should set ext id if unset' do
      @user.update_attribute(:voter_id_ext, nil)
      @user.register_voter
      @user.voter_id_ext.should_not == nil
    end
  end
  ...
  describe 'send_signup_email' do
    ...
    it 'should spawn email' do
      @user.send_signup_email
    end
  end

  describe 'responses counting' do
    fixtures :users
    ...
    before do
      ...
      @visits.times do
        visit = Visit.create!(:ip_address => '11', :user => @user)
        items = @items.times.map { Item.create!(:agree => '1', :visit => visit, :item_id_ext => count += 1, :name => 'name') }
        @responses.times do |i|
          response = Response.create!(:prompt_id => count += 1, :visit => visit, :vote_id_ext => count += 1, :response_time => 15)
          response.items << items if (i%2).zero?
        end
      end
    end
    
    describe 'responses count' do
      it 'should count all responses' do
        @user.responses_count.should == @responses
      end
    end
    
    describe 'item responses count' do
      it 'should count all item responses' do
        @user.item_responses_count.should == @responses / 2
      end
    end
  end
